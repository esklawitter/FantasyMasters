...
import dataset
import pandas as pd
from masters.models import Golfer
import copy
...
class PGADataExtractor(object):
    ...
    def refresh(self, force=False) -> dict:
        if force or (datetime.datetime.now() - self._last_refresh).total_seconds / 60.0 > self.refresh_freq:
            ...
            self.raw_leaderboard, self.field = self._compose_raw_board(response['leaderboard'])
    ...
    def _compose_raw_board(self, board:dict) -> pd.DataFrame:
        ...
        for player_data in board['players']:
            players.append(Golfer(player_data, PAR))
            #print(player.first_name, player.last_name, player.status, player.get_score_or_default([0,0,0,0]), player.get_next_tee_time())
        raw_board = pd.DataFrame([p.get_raw_score_dict() for p in players])
        return raw_board, players
